...
class AttributeSearch:
    ...
    async def run(self, inp: AttributeSearchInput) -> AttributeSearchOutput:
        ...
        """
        # The helper expects a raw AsyncElasticsearch (it issues nested KNN
        # queries with shapes specific to ES). ElasticClient.raw exposes it;
        # for tests that inject a mock implementing only `.search()`, we
        # fall back to the mock itself.
        raw_es = getattr(self._es, "raw", self._es)

        results = await _attribute_helpers.search_attributes(
            search_input=inp,
            # CVTextEmbedder duck-types as EmbedClient for the text-only path
            # the helper exercises; ditto the raw_es duck-type below.
            embed_client=cast("EmbedClient", self._embed),
            index=self._behavior_index,
            vst_external_url=self._vst_external_url,
            es=cast("AsyncElasticsearch", raw_es),
