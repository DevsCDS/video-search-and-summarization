...
"""SearchRuntime, RuntimeSnapshot, SearchOptions — the single env boundary.

This module is one of only two places in the library that may read env directly
(the other is cli.py). Primitives and clients NEVER read env; they receive a
SearchRuntime that was built by one of the builders here.
...
Four builders, three runtime-state shapes:
  ...
  SearchRuntime.from_config_file  — parse NAT-style YAML; preferred by vss-cli
                                    (only way to reproduce a deployed profile)
...
class SearchRuntime:
    ...
    def from_config_file(cls, path: str | Path, *, env: Mapping[str, str] | None = None) -> SearchRuntime:
        ...
        This is the **preferred** builder for vss-cli and any caller that must
        reproduce a deployed profile. Env alone cannot do that — profile-level
        settings live only in the NAT config:
        ...
        The deployed pod env already contains VSS_AGENT_CONFIG_FILE pointing at
        the active config; vss-cli uses that path by default.
        ...
        _critic_cfg = fns.get("critic_agent", {}) or {}  # reserved for future critic config

        base = cls.from_env(env)
        return dataclass_replace(
            base,
            # ES + behavior
            es_endpoint=embed_cfg.get("es_endpoint", base.es_endpoint),
            behavior_es_endpoint=search_cfg.get("behavior_es_endpoint", base.behavior_es_endpoint),
            video_embed_index=embed_cfg.get("es_index", base.video_embed_index),
            behavior_index=attr_cfg.get("behavior_index", base.behavior_index),
            # Attribute-search frame-lookup knobs (attribute_search.py:185-194)
            frames_index=attr_cfg.get("frames_index", base.frames_index),
            enable_frame_lookup=attr_cfg.get("enable_frame_lookup", base.enable_frame_lookup),
            # VST
            vst_internal_url=embed_cfg.get("vst_internal_url", base.vst_internal_url),
            vst_external_url=embed_cfg.get("vst_external_url", base.vst_external_url),
            # Embed clients
            cosmos_embed_endpoint=embed_cfg.get("cosmos_embed_endpoint", base.cosmos_embed_endpoint),
            embed_default_max_results=embed_cfg.get("default_max_results", base.embed_default_max_results),
            rtvi_cv_endpoint=attr_cfg.get("rtvi_cv_endpoint", base.rtvi_cv_endpoint),
            # Search orchestrator profile knobs — the whole point of this builder
            enable_critic=search_cfg.get("enable_critic", base.enable_critic),
            embed_confidence_threshold=search_cfg.get("embed_confidence_threshold", base.embed_confidence_threshold),
            search_max_iterations=search_cfg.get("search_max_iterations", base.search_max_iterations),
            default_max_results=search_cfg.get("default_max_results", base.default_max_results),
            fusion_method=search_cfg.get("fusion_method", base.fusion_method),
            w_attribute=search_cfg.get("w_attribute", base.w_attribute),
            w_embed=search_cfg.get("w_embed", base.w_embed),
            rrf_k=search_cfg.get("rrf_k", base.rrf_k),
            rrf_w=search_cfg.get("rrf_w", base.rrf_w),
            top_percent_filter=search_cfg.get("top_percent_filter", base.top_percent_filter),
        )
